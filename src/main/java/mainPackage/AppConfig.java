...
public class AppConfig 
{
	  ...
	  // public static String leaseFetchQuery  = "Select Company, Building,leaseName from Automation.InitialRentsUpdate where Status ='Pending' and Company ='Georgia'";
	   ...
	   public static String pendingLeasesQuery = "Select Company, Building, LeaseName, MonthlyRent, MonthlyRentFromPW, Status, Notes from [automation].[initialrentsupdate] where Status='Pending' and LeaseStatus Not in ('Dead Application','Terminated')  and company in ('Tennessee')";
	   
	   public static String failedLeasesQuery = "Select Company, Building,leaseName from Automation.InitialRentsUpdate where Status ='Failed'  and (Notes ='Building Not Found' or  Notes = ',Unable to Click Lease Onwer Name') and company in ('Tennessee')";	   
